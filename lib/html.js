// HTML assembly — podcast infographic with D3.js knowledge graph
...
export function assembleHtml(meta, chapters, analyses, synthesis, infographics = []) {
...
a:focus-visible,.kg-btn:focus-visible,.export-btn:focus-visible{outline:2px solid #ff6b4a;outline-offset:2px;border-radius:2px}
...
.top-nav .pod-label{display:none}
.top-nav .nav-links a{padding:0 10px;height:44px;font-size:.6rem;min-width:32px;justify-content:center}
.wrap{padding:0 16px 80px}
.hero{padding:64px 0 48px}
.hero h1{font-size:clamp(1.6rem,7vw,2.8rem)}
.hero .podcast-name{font-size:.7rem;margin-bottom:14px}
.thesis-box{padding:40px 0}
.thesis-box p{font-size:1.05rem;line-height:1.75}
.ch-section{padding:36px 0}
.ch-header h2{font-size:1.35rem}
.ch-summary p{font-size:.92rem;line-height:1.8}
.ch-infographic{margin:16px -16px 24px;overflow-x:auto;-webkit-overflow-scrolling:touch;padding:0 16px}
.ch-infographic svg{min-width:520px}
#kg-wrap{padding:40px 0}
.kg-frame{min-height:350px;height:60vh}
.kg-head{flex-direction:column;gap:12px}
.kg-copy h2,.section-title{font-size:1.8rem}
.chapter-list{padding:40px 0}
.chapter-list ul{columns:1}
