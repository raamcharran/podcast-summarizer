...
function isCodexEnvironment(env = process.env) {
  return Boolean(
    env.CODEX_SHELL ||
    env.CODEX_THREAD_ID ||
...
function detectProvider(env = process.env) {
  const explicit = canonicalizeProvider(env.AI_PROVIDER);
  if (explicit) return explicit;
  if (isCodexEnvironment(env)) return 'codex';
...
export function describeAiConfig(config = getAiConfig()) {
  const sessionProviderLabels = {
    'claude-cli': 'Claude Code session',
    codex: 'Codex session',
  };
  const label = sessionProviderLabels[config.provider];
  const modelPart = label
    ? (config.model === 'session-default' ? label : `${label}:${config.model}`)
...
async function runPromptCommand({ command, args, prompt, config, name }) {
  return new Promise((resolve, reject) => {
    ...
    proc.on('error', finish(error => reject(new Error(`${name} error: ${error.message}`))));
    proc.on('close', finish(code => {
      if (code !== 0) {
        const detail = (err || out).trim().slice(0, 300);
        reject(new Error(`${name} exited ${code}: ${detail}`));
      } else {
        const text = out.trim();
        if (!text) reject(new Error(`${name} returned no text`));
        else resolve(text);
      }
    }));
...
function buildCodexPrompt(system, prompt) {
  const codexSystem = [
    'You are being used as a text-only generation backend inside podcast-summarizer.',
    'Do not inspect files, read the repository, run shell commands, or use any tools.',
    'Answer only from the provided prompt and return just the requested content.',
  ].join(' ');
  return buildPrompt([codexSystem, system].filter(Boolean).join('\n\n'), prompt);
}

function getCodexExecArgs(config) {
  const args = ['-y', '@openai/codex', 'exec'];
  if (config.model && config.model !== 'session-default') {
    args.push('--model', config.model);
  }
  return args;
}

async function callCodex(config, system, prompt) {
  const fullPrompt = buildCodexPrompt(system, prompt);
  const args = getCodexExecArgs(config);

  if (process.platform === 'win32') {
    const npxCmd = path.join(path.dirname(process.execPath), 'npx.cmd');
    const psCommand = `& ${quotePowerShellArg(npxCmd)} ${args.map(quotePowerShellArg).join(' ')}`;
    return runPromptCommand({
...
export async function generateText({ system = '', prompt, maxTokens = 2048, config = getAiConfig() }) {
  return retryWithBackoff(() => {
    switch (config.provider) {
      case 'codex':
        return callCodex(config, system, prompt);
      case 'claude-cli':
